...
#for (k in 1:length(days)){
  for (j in 1:length(files)){
    ...
    for (i in 1:length(filename)){
      ...
      #DT_set <-DT_wflow %>%dials::parameters() %>%update(tree_depth = tree_depth(c(1, 30)))
    ...
    df <- rbind(df,va)
    s_result<-paste("C:/gln/mypackage/result/data_new_performance_py/R/nb/kknn/issueclosetime/",days[k],'/',files[j],".csv",sep='')
    ...
    s_para=paste("C:/gln/mypackage/result/data_new_performance_py/R/param/nb/kknn/issueclosetime/",days[k],'/',files[j],".csv",sep='')
