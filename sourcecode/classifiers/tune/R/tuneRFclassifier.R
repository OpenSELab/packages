...
#for (k in 1:length(days)){
  for (j in 1:length(files)){
    s<-paste('C:/gln/mypackage/dataset/dataset/data_new/smell/',files[j],'/','s_train/',sep='')
    ...
    for (i in 1:length(filename)){
      ...
      s1=paste('C:/gln/mypackage/dataset/dataset/data_new/smell/',files[j],'/','s_train/',filename[i],sep='')
      data_train<-read.csv(s1)
      data_train$Smell<-as.factor(data_train$Smell)
      data_train<-droplevels(data_train)
      s2=paste('C:/gln/mypackage/dataset/dataset/data_new/smell/',files[j],'/','s_test/',filename[i],sep='')
      data_test<-read.csv(s2)
      folds <- vfold_cv(data_train, v = 5)
      cell_pre_proc <-recipe(Smell ~ ., data = data_train) %>%step_downsample(Smell)
      rf_mod<-rand_forest(mode = "classification",mtry = tune(),trees = tune(),min_n = tune())%>%set_engine("randomForest")
      
      rf_wflow <-workflow() %>%add_model(rf_mod) %>%add_recipe(cell_pre_proc)
      
      rf_set <-rf_wflow %>%parameters() %>%update(mtry = mtry(range = c(1L, 10L)),
                                                  trees = trees(c(100, 500)))
      rf_search_2 <- tune_bayes(rf_wflow, resamples = folds,iter = 100,metrics = metric_set(roc_auc),param_info = rf_set,control = control_bayes(verbose = TRUE))
      ...
      meas<-confusionMatrix(y_pred1,as.factor(y_true),positive='1', mode = "prec_recall")
    ...
    df <- rbind(df,va)
    s_result<-paste("C:/gln/mypackage/result/data_new_performance_py/R/nb/rf/Smell/",files[j],".csv",sep='')
    ...
    s_para=paste("C:/gln/mypackage/result/data_new_performance_py/R/param/nb/rf/Smell/",files[j],".csv",sep='')
