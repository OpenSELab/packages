...
filename <- list.files("E:/SDP/Classifiers/synapse/s_train/")
df<- data.frame()
df_para<-data.frame()
...
for (i in 1:length(filename)){
  ...
  s1=paste("E:/SDP/Classifiers/synapse/s_train/",'/',filename[i],sep='')
  data_train<-read.csv(s1)
  data_train$label<-as.factor(data_train$label)
  #data_train<-droplevels(data_train)
  s2=paste("E:/SDP/Classifiers/synapse/s_test/",'/',filename[i],sep='')
  ...
  knn_search_2 <- tune_bayes(knn_wflow, resamples = folds,iter = 100,metrics = metric_set(roc_auc),param_info = knn_set,control = control_bayes(verbose = TRUE))
...
df <- rbind(df,va)
write.table(df,"E:/SDP/TuneClassifiers/result/R/KNN/synapse.csv",row.names=FALSE,col.names=TRUE,sep=",")
...
write.table(df_para,"E:/SDP/TuneClassifiers/result/Parameters/KNN/synapse.csv",row.names=FALSE,col.names=TRUE,sep=",")