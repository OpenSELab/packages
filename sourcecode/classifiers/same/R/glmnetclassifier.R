...
for (k in 1:length(days)){
  ...
  for (i in 1:length(filename)){
    ...
    eval_function_XGBoost_classifier <- function(x,data,train_settings) {
      ...
      XGBoost_classifier_model <- caret::train(Traindata, Trainclass,
                                               ...
                                               tuneGrid = expand.grid(
                                                 alpha = x1, # number of boosting iterations
                                                 lambda=x2
                                               ))
    ...
    lower = c(alpha[1],lambda[1])
    upper = c(alpha[2],lambda[2])
    ...
    dt_alpha[i]<-DE_solutions[1]
    dt_lambda[i]<-DE_solutions[2]
    #dt_eta[i]<-DE_solutions[3]
    
    # Grid of optimal hyperparameter values
    DE_XGBoost_classifier_grid <- expand.grid(
      alpha = round(DE_solutions[1]), 
      lambdah=round(DE_solutions[2])
    )
  ...
  df <- rbind(df,va)
  s_result<-paste("E:/gln/C/mypackage/result_new/RQ2/R/tune/DE/glmnet/SDP/",days[k],".csv",sep='')
  write.table(df,s_result,row.names=FALSE,col.names=TRUE,sep=",")
  va_para<-cbind(dt_alpha,dt_lambda)
  names_para <- c("alpha","lambda")
  df_para<- data.frame()
  df_para[[names_para[1]]] <- as.numeric()
  df_para <- rbind(df_para,va_para)
  s_para=paste("E:/gln/C/mypackage/result_new/RQ2/R/tune/DE/param/glnmet/SDP/",days[k],".csv",sep='')
