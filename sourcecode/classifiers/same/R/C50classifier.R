...
for (k in 1:length(days)){
  ...
  for (i in 1:length(filename)){
    ...
    eval_function_XGBoost_classifier <- function(x,data,train_settings) {
      ...
      XGBoost_classifier_model <- caret::train(Traindata, Trainclass,
                                               ...
                                               tuneGrid = expand.grid(
                                                 trials = round(x1), # number of boosting iterations
                                                 model="tree",
                                                 winnow = FALSE
                                               ))
    ...
    DE_XGBoost_classifier_grid <- expand.grid(
      trials = round(DE_solutions[1]), # number of boosting iterations
      model="tree",
      winnow = FALSE
    )
  ...
  df <- rbind(df,va)
  s_result<-paste("E:/gln/C/mypackage/result_new/RQ2/R/tune/DE/C50/SDP/",days[k],".csv",sep='')
 ...
 # colnames(df_para)<-c("trials")
  s_para=paste("E:/gln/C/mypackage/result_new/RQ2/R/tune/DE/param/C50/SDP/",days[k],".csv",sep='')
