...
for (k in 1:length(days)){
  for (j in 1:length(files)){
    ...
    for (i in 1:length(filename)){
      ...
      rf_model<-randomForest(label~.,data=data_train)
      
      
      y_pred <- predict(rf_model, data_test)
      y_prob<-predict(rf_model,data_test,type = "prob")
    ...
    df <- rbind(df,va)
    s_result<-paste("E:/gln/C/mypackage/result_new/RQ1/defaultsetting/RF/SDP/randomforest/",days[k],".csv",sep='')
