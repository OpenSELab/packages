...
for (j in 1:length(days)){
  #for (k in 1:length(files)){
    ...
    for (i in 1:length(filename)){
      ...
      rpart_classifier_model<-rpart(label~.,data=data_train)

      y_pred <- predict(rpart_classifier_model, data_test,type = "class")
      y_prob<-array(as.data.frame(predict(rpart_classifier_model,data_test,type = "prob")))
    ...
    df <- rbind(df,va)
    s_result<-paste("E:/gln/C/mypackage/result_new/RQ1/defaultsetting/DT/SDP/",'/rpart/',days[j],".csv",sep='')
