...
for (k in 1:length(days)){
  for (j in 1:length(files)){
    ...
    for (i in 1:length(filename)){
      ...
      glm_model<-glm(issude_close_time~.,family='binomial',data=data_train)
      
      y_prob <- predict(glm_model, data_test,"response")
    ...
    df <- rbind(df,va)
    s_result<-paste("E:/gln/C/mypackage/result_new/RQ1/defaultsetting/LR/issueclosetime/",days[k],"/glm/",files[j],".csv",sep='')
