...
for (k in 1:length(days)){
  ...
  dt_k<-vector(mode="numeric",length=100)
  ...
  for (i in 1:length(filename)){
    ...
    eval_function_XGBoost_classifier <- function(x,data,train_settings) {
      ...
      XGBoost_classifier_model <- caret::train(Traindata, Trainclass,
                                               ...
                                               tuneGrid = expand.grid(
                                                 kmax = round(x1), # number of boosting iterations
                                                 distance=2,
                                                 kernel="gaussian"
                                               ))
    ...
    kmax <- c(1,20)
    distance<-c(2,2)
    kernel<-c("rectangular","rectangular")
    
    lower = c(kmax[1],distance[1],kernel[1])
    upper = c(kmax[2],distance[2],kernel[2])
  ...
  df <- rbind(df,va)
  s_result<-paste("E:/gln/C/mypackage/result_new/RQ2/R/tune/DE/kknn/SDP/",days[k],".csv",sep='')
  write.table(df,s_result,row.names=FALSE,col.names=TRUE,sep=",")
  va_para<-cbind(dt_k,dt_kernel)
  names_para <- c("k","kernel")
  df_para<- data.frame()
  df_para[[names_para[1]]] <- as.numeric()
  df_para[[names_para[2]]] <- as.numeric()
  df_para <- rbind(df_para,va_para)
  #colnames(df_para)<-c("k","kernel")
  s_para=paste("E:/gln/C/mypackage/result_new/RQ2/R/tune/DE/param/kknn/SDP/",days[k],".csv",sep='')
