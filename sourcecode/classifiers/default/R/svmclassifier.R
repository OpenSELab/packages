...
for (k in 1:length(days)){
  for (j in 1:length(files)){
    ...
    dt_C<-vector(mode="numeric",length=100)
    ...
    for (i in 1:length(filename)){
      ...
      sv_model<-svm(issude_close_time~ ., data = data_train,probability=TRUE)
      
      y_pred <- predict(sv_model, data_test)
      y_prob<-predict(sv_model,newdata=data_test,probability = TRUE)
      y_prob<-attr(y_prob,"probabilities")
    ...
    df <- rbind(df,va)
    s_result<-paste("E:/gln/C/mypackage/result_new/RQ1/defaultsetting/SVM/issueclosetime/",days[k],"/SVM/",files[j],".csv",sep='')
