...
for (j in 1:length(days)){
  #for (k in 1:length(files)){
    s<-paste("E:/gln/C/mypackage/dataset/dataset/dataset/",days[j],'/',"s_train/",sep='')
    ...
    for (i in 1:length(filename)){
      ...
      s1=paste("E:/gln/C/mypackage/dataset/dataset/dataset/",days[j],'/',"s_train/",filename[i],sep='')
      ...
      s2=paste("E:/gln/C/mypackage/dataset/dataset/dataset/",days[j],'/',"s_test/",filename[i],sep='')
      data_test<-read.csv(s2)
      Testdata<-data_test[,-ncol(data_test)]
      Testclass<-data_test$label
      
      
      ctree_classifier_model <-ctree(label~.,data=data_train,control =ctree_control(maxdepth=30,minsplit= 20))
      
      
      y_pred <- predict(ctree_classifier_model, Testdata)
      y_prob<-predict(ctree_classifier_model,Testdata,type = "prob")
      y_prob<-data.frame(matrix(unlist(y_prob), nrow=length(y_prob), byrow=T),stringsAsFactors=FALSE)
      ...
      y_pred1<-as.factor(as.numeric(y_prob[,2][2] > 0.5))
    ...
    df <- rbind(df,va)
    s_result<-paste("E:/gln/C/mypackage/result_new/RQ1/samesetting/DT/SDP/ctree/",days[j],".csv",sep='')
