...
def tunparameter(x_train,y_train):
    param_range_trees = range(200, 600, 200)
    # param_range_features = [3, 5, 10, 15]
    param_grid = [{'clf__n_estimators': param_range_trees}]
...
def fun(file):
    ...
    for t in range(100):
        ...
        pipe_rf = Pipeline([('clf', RandomForestClassifier(criterion='gini', random_state=1, n_jobs=1))])
        param_range_trees = range(200, 600, 200)
        # param_range_features = [3, 5, 10, 15]
        param_grid = [{'clf__n_estimators': param_range_trees}]
    ...
    ss='C:/gln/myclassoverlap/imbalanceoverlap/result/feature/RF/original/original_'+file
    feature_original.to_csv(ss,index=False)
    ss1='C:/gln/myclassoverlap/imbalanceoverlap/result/feature/RF/remove/remove_'+file
    feature_remove.to_csv(ss1,index=False)
    ss2 = 'C:/gln/myclassoverlap/imbalanceoverlap/result/feature/RF/SMOTE/SMOTE_' + file
    feature_SMOTE.to_csv(ss2, index=False)
    ss3 = 'C:/gln/myclassoverlap/imbalanceoverlap/result/feature/RF/ovim/ovim_' + file
    feature_ovim.to_csv(ss3, index=False)
    ss4 = 'C:/gln/myclassoverlap/imbalanceoverlap/result/feature/RF/imov/imov_' + file
    feature_imov.to_csv(ss4, index=False)
    ss5 = 'C:/gln/myclassoverlap/imbalanceoverlap/result/feature/RF/NCL/NCL_' + file
    feature_NCL.to_csv(ss5, index=False)
    ss6 = 'C:/gln/myclassoverlap/imbalanceoverlap/result/feature/RF/IKMCCA/IKMCCA_' + file
