...
def tunparameter(x_train,y_train):
    param_range_n_neighbors = range(1, 20, 5)
    param_grid = [{'clf__n_neighbors': param_range_n_neighbors}]
    pipe_rf = Pipeline([('clf', KNeighborsClassifier())])
    best_score = 0
...
def fun(file):
    ...
    for t in range(100):
        ...
        pipe_rf = Pipeline([('clf', KNeighborsClassifier())])
        param_range_n_neighbors = range(2, 20, 5)
        param_grid = [{'clf__n_neighbors': param_range_n_neighbors}]
    ...
    ss='C:/gln/myclassoverlap/imbalanceoverlap/result/feature/DT/original/original_'+file
    feature_original.to_csv(ss,index=False)
    ss1='C:/gln/myclassoverlap/imbalanceoverlap/result/feature/DT/remove/remove_'+file
    feature_remove.to_csv(ss1,index=False)
    ss2 = 'C:/gln/myclassoverlap/imbalanceoverlap/result/feature/DT/SMOTE/SMOTE_' + file
    feature_SMOTE.to_csv(ss2, index=False)
    ss3 = 'C:/gln/myclassoverlap/imbalanceoverlap/result/feature/DT/ovim/ovim_' + file
    feature_ovim.to_csv(ss3, index=False)
    ss4 = 'C:/gln/myclassoverlap/imbalanceoverlap/result/feature/DT/imov/imov_' + file
    feature_imov.to_csv(ss4, index=False)
    ss5 = 'C:/gln/myclassoverlap/imbalanceoverlap/result/feature/DT/NCL/NCL_' + file
    feature_NCL.to_csv(ss5, index=False)
    ss6 = 'C:/gln/myclassoverlap/imbalanceoverlap/result/feature/DT/IKMCCA/IKMCCA_' + file
