...
for (j in 1:length(files)){
  ...
  for (i in 1:length(filename)){
    ...
    classifier_model_randomForest<- randomForest(label~.,data=train,ntree=ntree[i],mtry=mtry[i])
    mode_randomForest<-Predictor$new(classifier_model_randomForest, data =Testdata, y = Testclass)
    imp_randomForest<-FeatureImp$new(mode_randomForest, loss = "ce")
    test_features_randomforest[,i]<-imp_randomForest$results$importance
  ...
  s_randomforest<-paste("E:/gln/C/mypackage/result_new/RQ3/featureimportant/samesetting/RF/randomforest/",files[j],'.csv',sep='')
